...
export function AskLoading({ query }: { query?: string }) {
  const [index, setIndex] = useState(0);
  const [stored, setStored] = useState("");
  const heading = query || stored;

  useEffect(() => {
    setStored(sessionStorage.getItem("aarla:last-prompt") ?? "");
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 xl:max-w-5xl xl:px-10">
      ...
      {heading ? (
        <p className="mt-3 max-w-2xl font-serif text-3xl leading-snug xl:text-4xl">{heading}</p>
      ) : (
